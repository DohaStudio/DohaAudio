...
Production 구성은 selection record, secret-free config, explicit factory와 adapter로 분리합니다. DohaMusic ADR-038을 authority로 downstream model `DOHAMUSIC_DELEGATED_ASSERTION`을 선택했으며 external IdP는 null입니다. 현재 config가 없으므로 factory readiness는 selected true, configured·operational false이고, synthetic delegated config도 proof를 발급하지 않는 unavailable stub만 구성합니다. Factory는 selection/config 없음, disabled·unknown·mismatch와 fake production config를 fail-closed하며 fallback하지 않습니다. Persistent private mapping implementation은 `ReviewerIdentityMappingStore` protocol 뒤에 두고 public `ReviewerAuthorityRegistry`와 합치지 않습니다.
